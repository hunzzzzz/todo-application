...
class TodoService(val repository: TodoRepository) {
    ...
    fun getTodoById(id: Long) =
        entityToResponse(repository.findByIdOrNull(id) ?: throw IllegalStateException("존재하지 않는 Todo 입니다."))
    ...
    fun deleteTodo(id: Long) = repository.delete(
        repository.findByIdOrNull(id) ?: throw IllegalStateException("존재하지 않는 Todo 입니다.")
    )
}